...
fn batch_processes_all_matching_files() {
    ...
    let out = pdtk_output(&["batch", dir.path().to_str().unwrap(), "validate"]);
    assert!(
        out.contains("3/3 succeeded"),
        "all 3 files should succeed: {out}"
    );
}
...
fn batch_glob_filter_correct() {
    ...
    for name in &["keep.pd", "skip.pd", "also.pd"] {
        std::fs::write(dir.path().join(name), "#N canvas 0 22 450 300 12;\n").unwrap();
    }
    ...
    assert!(
        out.contains("1/1 succeeded"),
        "glob should match only keep.pd: {out}"
    );
...
fn batch_error_stops_by_default() {
    ...
    let out = run_pdtk(&["batch", dir.path().to_str().unwrap(), "validate"]);
...
fn batch_continue_on_error_processes_remaining() {
    ...
    assert!(
        s.contains("a.pd") && s.contains("b.pd"),
        "both files should appear: {s}"
    );
}
...
fn batch_json_output_schema() {
    ...
    let out = pdtk_output(&["batch", dir.path().to_str().unwrap(), "--json", "validate"]);
...
fn batch_validate_all_handcrafted_fixtures() {
    ...
    assert!(
        ...
        "batch should report successes: {s}"
    );
    let _ = total;
}
